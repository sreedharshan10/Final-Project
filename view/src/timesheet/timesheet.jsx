...
function Timesheet() {
    ...
    useEffect(() => {
        const fetchProjectAllocations = async () => {
            try {
                const response = await fetch('http://localhost:3001/api/project-allocations');
                if (!response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        const weekEndDate = formatDate(new Date(selectedWeekStart.getTime() + 6 * 24 * 60 * 60 * 1000));
        ...
        try {
            for (let i = 0; i < rows.length; i++) {
                const row = rows[i];
                const selectedProject = projectAllocations.find(project => project.projectName === row.selectedProject);
                if (!selectedProject) {
                ...
                const response = await axios.post('http://localhost:3002/api/timesheets', timesheetData, {
                    headers: {
    ...
    const calculateTotal = () => {
        return rows.reduce((total, row) => {
            return total + row.hoursBAU.reduce((sum, hour) => sum + hour, 0) + row.hoursSales.reduce((sum, hour) => sum + hour, 0);
        }, 0);
    };
...
>
    ...
    {projectAllocations
        .filter(project => project.userDetails.some(user => user.userId === userId))
        .map(project => (
                ...
                {project.projectName}
                                    ...
                                    <td>
                                        ...
                                        >
                                            ...
                                            {row.selectedProject &&
                                                projectAllocations
                                                    .filter(project => project.projectName === row.selectedProject)
                                                    .map(project => project.userDetails
                                                        .filter(user => user.userId === userId)
                                                        .map(user => user.userTasks.map(task => (
...
/>
                                    ...
                                    {row.hoursBAU.map((hour, dayIndex) => (
                                        <td key={dayIndex}>
                                            <input
                                                ...
                                                style={{ width: "55px" }}
                                                value={hour}
                                                onChange={(e) => handleInputChangeBAU(rowIndex, dayIndex, e.target.value)}
                                            />
                                        </td>
                                    ))}
                                    ...
                                    <td>
                                        {rowIndex === 0 ? (
                                            <button
                                                style={{ backgroundColor: "#720455", color: "white", border: "none", cursor: "pointer" }}
                                                onClick={addRow}
                                            >
                                                +
                                            </button>
                ...
                <div className="flex-container" style={{ justifyContent: 'flex-end', marginTop: '10px' }}>
                    <button id="save" style={{ marginRight: "10px", backgroundColor: "#720455", color: "white", border: "none", cursor: "pointer" }} >Save</button>
                    <button id="submit" style={{ marginRight: "10px", backgroundColor: "#720455", color: "white", border: "none", cursor: "pointer" }} onClick={handleSubmit} type="submit">Submit</button>
                </div>
            </div>
        </form>
    );
